...
// errorsMessagesBody is the spec-compliant error body: {"errorsMessages": [...]}
type errorsMessagesBody struct {
	ErrorsMessages []apperrors.FieldError `json:"errorsMessages"`
}

// CustomHTTPErrorHandler handles all errors returned from handlers and middleware.
// It produces a unified JSON error response in spec format:
//
...
func CustomHTTPErrorHandler(err error, c echo.Context) {
	...
	if errors.As(err, &appErr) {
		...
		_ = c.JSON(appErr.Code, errorsMessagesBody{ErrorsMessages: appErr.ErrorsMessages()})
		return
	}
	...
	if errors.As(err, &echoErr) {
		...
		_ = c.JSON(code, errorsMessagesBody{ErrorsMessages: []apperrors.FieldError{{Message: message}}})
		return
	}

	// 3. Unknown errors — log and return generic 500
	c.Logger().Errorf("Unhandled error: %v", err)
	_ = c.JSON(http.StatusInternalServerError, errorsMessagesBody{
		ErrorsMessages: []apperrors.FieldError{{Message: "Internal server error"}},
	})
}
