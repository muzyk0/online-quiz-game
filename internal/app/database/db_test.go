...
func TestNewDB(t *testing.T) {
	...
	db, err := New(context.Background(), "invalid-url", 25, 5, 5)
	require.Error(t, err)
	assert.Nil(t, db)
	...
	db, err = New(context.Background(), "postgres://user:password@localhost:5432/nonexistent_db?sslmode=disable", 25, 5, 5)
	if err != nil {
